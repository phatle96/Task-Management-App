...
describe('Task API testing', () => {
    ...
    describe('GET /task/:task_id', () => {
        test("respond with 404", async () => {
            const task_id = 'task_1234';
            const res = await request(app).get(`/api/task/${task_id}`)
        ...
        test("respond with 200", async () => {
            ...
            const res = await request(app).get(`/api/task/${task.task_id}`)
                .set('Accept', 'application/json');
    ...
    describe('POST /task/create', () => {
        test("respond with 200", async () => {
            ...
            const res = await request(app).post('/api/task/create')
                .send(task_payload)
                .set('Accept', 'application/json');
            expect(res.status).toBe(200);
    ...
    describe('POST /task/create', () => {
        test("respond with 400", async () => {
            const payload = this.payload.error_task;
            const res = await request(app).post('/api/task/create')
                .send(payload)
                .set('Accept', 'application/json');
    ...
    describe('PUT /task/:id/update', () => {
        test('response with 200', async () => {
            ...
            const list = await List.create(list_payload)
            const task = await Task.create({ ...task_payload, list: list });
            const updated_task_payload = { ...this.payload.updated_task, list: list._id };
            const res = await request(app).put(`/api/task/${task.task_id}/update`)
                .send(updated_task_payload)
                .set('Accept', 'application/json');
            expect(res.status).toBe(200);
            expect(res.body).toEqual({
                __v: 0,
                ...
                person: [],
                is_completed: false,
                content: updated_task_payload.content,
                is_deleted: false,
                createdAt: expect.any(String),
                updatedAt: expect.any(String),
            });
    ...
    describe('DELETE /:task_id/delete', () => {
        test('respond with 200', async () => {
            // declare variable
            const list_payload = this.payload.valid_list;
            const task_payload = this.payload.valid_task;
            ...
            // create some test task, task, subtask
            const list = await List.create(list_payload);
            const task = await Task.create({ ...task_payload, list: list });
            ...
            const subtask_2 = await Subtask.create({ ...subtask_payload, list: list, task: task_2 });

            // create delete request
            const res = await request(app).delete(`/api/task/${task.task_id}/delete`)
                .set('Accept', 'application/json');
            expect(res.status).toBe(200);
    ...
    describe('DELETE /:task_id/delete', () => {
        test('respond with 404', async () => {
            ...
            const list_payload = this.payload.valid_list;
            const list = await List.create(list_payload);
            const task_payload = this.payload.deleted_task;
            const task = await Task.create({ ...task_payload, list: list });
